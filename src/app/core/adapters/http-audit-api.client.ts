...
export class HttpAuditApiClient implements AuditApiClient {
      ...
      .pipe(
        map((response) => {
          ...
          const result: FindingRemediation = {
            ...
            proposedHtml: diffLines || `<div class="target-element" ${proposal.suggestedDiff || ''}>...</div>`,
            explanation: `${proposal.title || 'Remediation'}: ${proposal.description || 'Accessibility fix'}`,
            apgPattern: proposal.suggestedPattern || 'dialog'
          };
          return result;
        }),
        catchError(() => {
          const fallback = this.generateContextualRemediation(request.findingId);
          return of(fallback);
        })
      );
  }
  ...
  private generateContextualRemediation(findingId?: string): FindingRemediation {
